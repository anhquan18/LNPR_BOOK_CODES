...
class IdealRobot(object):   
    ...
    def draw_coordinate_system(self, ax):   
        origin = self.pos_trans_to_world(np.array([0, 0]).T) 
        for v in [[1, 0], [0, 1]]:                          
            wv = self.vec_trans_to_world(np.array(v).T)     
            ax.quiver(origin[0], origin[1], wv[0], wv[1],  
...
class Landmark(object):
    def __init__(self, x, y):
        self.pose = np.array([x, y]).T
        self.id = None
        
    def draw(self, ax, elems):
        c = ax.scatter(self.pose[0], self.pose[1], s=100, marker="*", label="landmarks", color="orange")
        elems.append(c)
        elems.append(ax.text(self.pose[0], self.pose[1], "id:" + str(self.id), fontsize=10))


class RobotLandMark(object):
    def __init__(self, x, y, th, color):
        self.pose = np.array([x, y, th]).T
        self.color = color
        self.r = 0.3
        self.distance_range = (0.3, 6.1)
        self.direction_range = (-math.pi/5,  math.pi/5)
        self.id = None

    def draw(self, ax, elems):
        xn = self.pose[0] + self.r * math.cos(self.pose[2])  
        yn = self.pose[1] + self.r * math.sin(self.pose[2])

        elems += ax.plot([self.pose[0], xn], [self.pose[1], yn], color=self.color)
        c = patches.Circle(xy=(self.pose[0], self.pose[1]), radius=self.r, fill=False, color=self.color) 
        elems.append(ax.add_patch(c))
        elems.append(ax.text(self.pose[0], self.pose[1], "ROBOT:" + str(self.id), fontsize=24))

    def visible(self, obj_pos):
        if obj_pos is None:
            return False

        return self.distance_range[0] < obj_pos[0] < self.distance_range[1] and self.direction_range[0] < obj_pos[1] < self.direction_range[1]
    ...
    def relative_polar_pos(cls, cam_pose, obj_pos):
        ...
        relative_pos = np.array([[c,  s],
                                [-s, c]]).dot(obj_pos[0:2] - cam_pose[0:2])
